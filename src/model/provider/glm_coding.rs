...
//!
//! This provider uses the GLM Coding Plan endpoint which requires a separate subscription.
//!
//! # Endpoint
...
//!
//! # Notes
//! - The Coding Plan endpoint is only available with a valid GLM Coding Plan subscription
//! - Request/response format is identical to the standard GLM API
//! - Use this provider when you have purchased the GLM Coding Plan for coding tools
//! - For general API usage, use `GlmProvider` instead
...
impl GlmCodingPlanProvider {
    /// Create a new GLM Coding Plan provider
    ///
    /// # Arguments
    /// - `model`: Model name (e.g., "glm-4.7", "glm-4.7-flashx")
    ...
    ///
    /// let provider = GlmCodingPlanProvider::new("glm-4.7", "your-api-key")
    ///     .with_base_url("https://custom-endpoint.com/chat/completions");
    /// ```
...
#[async_trait]
impl ModelClient for GlmCodingPlanProvider {
    async fn chat(
        &self,
...
mod tests {
    ...
    fn test_default_base_url() {
        let provider = GlmCodingPlanProvider::new("glm-4.7", "test-key");
        assert_eq!(
            provider.base_url,
    ...
    fn test_custom_base_url() {
        let provider = GlmCodingPlanProvider::new("glm-4.7", "test-key")
            .with_base_url("https://custom.api.com/v4/chat/completions");
        assert_eq!(
            provider.base_url,
            "https://custom.api.com/v4/chat/completions"
        );
    }

    #[test]
    fn test_truncate_response_short() {
        let text = "Short response";
        assert_eq!(truncate_response(text), text);
    }

    #[test]
    fn test_truncate_response_long() {
